...
export const BubbleChart: React.FC<BubbleChartProps> = ({
  ...
  // Tooltip state
  const [tooltip, setTooltip] = useState<null | { x: number; y: number; label?: string; value: { x: number; y: number; r: number } }>(null);
  ...
  return (
    <div className="bubble-chart-responsive" style={{ width: "100%", maxWidth: width }}>
      <svg
        width="100%"
        height={height}
      ...
      >
        ...
        <AnimatePresence>
          {data.map((point, i) => (
            <motion.circle
              ...
              animate={{ r: point.r, opacity: 0.85 }}
              exit={{ r: 0, opacity: 0 }}
              transition={{ duration: animationDuration / 1000, type: "spring", stiffness: 120 }}
              tabIndex={0}
              aria-label={point.label || `Bubble (${point.x}, ${point.y}, r=${point.r})`}
              onMouseEnter={() => setTooltip({ x: scaleX(point.x), y: scaleY(point.y), label: point.label, value: { x: point.x, y: point.y, r: point.r } })}
              onFocus={() => setTooltip({ x: scaleX(point.x), y: scaleY(point.y), label: point.label, value: { x: point.x, y: point.y, r: point.r } })}
        ...
        {/* Tooltip */}
        {tooltip && (
          <foreignObject x={tooltip.x + 10} y={tooltip.y - 40} width={140} height={50} pointerEvents="none">
            <div className="bubble-tooltip" style={{ background: "rgba(0,0,0,0.8)", color: "#fff", borderRadius: 6, padding: "6px 10px", fontSize: 13, pointerEvents: "none" }}>
              {tooltip.label && <div style={{ fontWeight: 600 }}>{tooltip.label}</div>}
              <div>X: {tooltip.value.x}</div>
              <div>Y: {tooltip.value.y}</div>
              <div>Radius: {tooltip.value.r}</div>
            </div>
          </foreignObject>
        )}
      </svg>
